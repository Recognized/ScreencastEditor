...
import com.github.recognized.screencast.editor.lang.script.psi.RawTreeNode.IndexedEntry.CodeEntry
import com.github.recognized.screencast.editor.lang.script.psi.RawTreeNode.IndexedEntry.Offset
...
class RawTreeNode(var data: RawTreeData) : EditableTreeNode, PrintableTreeNode {
  ...
  companion object {
    ...
    fun buildPlainTree(root: RawTreeNode, oldRange: IntRange? = null): List<IndexedEntry> {
      ...
      if (oldRange != null) {
        if (lastOffset < oldRange.endInclusive) {
          list.add(Offset(index, oldRange.endInclusive))
        }
      }
      return list
    }
    ...
    fun toCodeModel(root: RawTreeNode, oldRange: IntRange? = null): CodeModel {
      ...
      val (codes, _) = fold<Code>(code, 0) { entry, inner ->
        ...
        val correctedRange = (range.start + range.length / total * (k - 1))..range.endInclusive
    ...
    private fun <T> fold(
      ...
      operation: (CodeEntry, List<T>) -> T
    ): Pair<List<T>, Int> {
      val result = mutableListOf<T>()
      var index = startIndex
      while (index < code.size) {
        val x = code[index++]
        if (x.indexRange.endInclusive > until) {
          return result to index - 1
        }
        if (!x.isBlock) {
          result.add(operation(x, listOf()))
        } else {
          val (res, newIndex) = fold(code, index, x.indexRange.endInclusive, operation)
          index = newIndex
    ...
    private fun markRawTreeOutput(
      expressions: List<CodeEntry>,
      offsets: List<Offset>
    ): Output {
      val intervals = offsets.sortedBy { it.indexRange.start }
      var j = 0
      val time = mutableMapOf<CodeEntry, IntRange>()
      val fraction = mutableMapOf<CodeEntry, Pair<Int, Int>>()
      val sameRangeElements = mutableMapOf<IntRange, MutableList<CodeEntry>>()
      out@ for (expr in expressions) {
        while (j < intervals.size) {
          val interval = intervals[j]
          if (interval.indexRange.endInclusive <= expr.indexRange.start) {
            while (j < intervals.size && intervals[j].indexRange.endInclusive <= expr.indexRange.start) j++
            j--
            var i = j + 1
            while (i < intervals.size) {
              if (expr.indexRange.endInclusive <= intervals[i].indexRange.start) {
