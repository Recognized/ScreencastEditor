...
class WaveformModel(
  ...
  data class WordView(val word: WordData, val pixelRange: IntRange) {
    ...
    val rightBorder =
      IntRange(pixelRange.endInclusive - JBUI.scale(MAGNET_RANGE), pixelRange.endInclusive + JBUI.scale(MAGNET_RANGE))
  }
  ...
  fun getCoveredRange(extent: IntRange): IntRange {
    ...
    val left = coordinates.find { it.endInclusive >= extent.start }?.start ?: return IntRange.EMPTY
    val right = coordinates.findLast { it.start <= extent.endInclusive }?.endInclusive ?: return IntRange.EMPTY
    return left..right
  }
